    ...
    <section id="skills" className="py-24 bg-slate-950 relative overflow-hidden scroll-mt-28">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          <h3 className="text-3xl md:text-4xl font-bold text-white">技術スタック</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {categories.map((category, catIndex) => (
            <div key={category} className="space-y-6 animate-fade-in-up" style={{ animationDelay: `${catIndex * 0.1}s` }}>
              <h4 className="text-xl font-bold text-slate-200 border-b border-slate-800 pb-2 flex items-center justify-between">
                {category}
                <span className="text-xs font-normal text-slate-500">0{catIndex + 1}</span>
              </h4>
