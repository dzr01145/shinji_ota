...
### Step 1: 情報収集方針の事前ヒアリングと決定 ★必須★

ブログ生成の依頼を受けた際、**いきなり記事の収集・生成エンジンを回し始めず、必ず以下の2点についてユーザーに事前確認を行うこと**。

1. **情報収集ツールの選択**:
   - `NotebookLM` (登録済みノートベースやMCP深堀りリサーチ)
   - `FIRECRAWL` (APIを使用した指定キーワード・ドメインによる最新Webスクレイピング)
   - または、その両方の併用
2. **収集の的（ターゲット）**:
   - 厚生労働省（mhlw.go.jp）や労働安全衛生総合研究所など「国・公的機関のサイト」を中心に狙うか？
   - 最新の「労働災害の実事例ニュース」を中心に抽出するか？
   - その他、特定の競合を排除したり指定のドメインに絞るなどの希望はあるか？

質問例：
> 「情報収集について確認させてください。今回は NotebookLM と FIRECRAWL のどちらを優先して使いますか？また、厚生労働省などの公的機関を中心にするか、最新の事故ニュースサイトにするかなど、収集する記事のターゲット（方針）をお知らせください。」

ユーザーからの回答を得て収集ツールとターゲットが確定してから、Step 2（実際の情報収集）へ進むこと。（ユーザーがすでに最初からツールやターゲットを明確に指定している場合は、その指示に従って直接進めてよい）
...
1. `mcp__notebooklm-mcp-server__research_start` でリサーチを開始する
   ...
   - `query`: トピックに関連するキーワード（日本語可）に加え、「**直近1年以内の具体的な労災ニュース事例（いつ、どこで、どんな事故が起きたかという実際の報道内容）を必ず検索して含めること**」と明示的に指示をする。
   - `source`: `"web"`
...
4. `mcp__notebooklm-mcp-server__notebook_query` でインポートされたソースに対して質問し、統計データ・事故事例・法令情報・出典URLを取得
   - `notebook_id`: `c5f67b8a-625d-473c-ad8d-36141d665fbf`
   - `query`: 質問文（「**実際に報道された具体的な年月日・場所・事故内容を含むリアルなニュース事例を最低1つは抽出し、その詳細をまとめること**」と明記する）
...
- マークダウン使用：`##` 見出し、`**太字**`、`- リスト`
- 冒頭で読者の課題感に触れる
- **必ず直近（過去1〜2年以内）の「実際に発生した労災ニュース事例（〇年〇月、〇〇県の〇〇工場で〜）」を最低1箇所組み込み、リアリティのある教訓として活用する**
- 具体的な数字・法令名を含める
- 末尾に「まとめ」または行動を促す一文
- **本文末尾に必ず `## 参考資料` セクションを設ける。ここでは、記事内で言及した「実際の労災ニュース事例（最低でも3つ）」の検索リンクを必ず含めること。ニュースリンクはリンク切れになりやすいため、元のURLではなく `https://duckduckgo.com/?q=ページタイトル` のような検索URLを使用する**
...
1. **インフォグラフィック生成の指示**:
   - `mcp__notebooklm-mcp-server__studio_create` を実行（`artifact_type="infographic"`, `orientation="landscape"` (16:9相当)）。
   - **プロンプト指定（重要）**: 「[記事テーマ]に関するインフォグラフィック。**テキストラベルは一切含めないか、できるだけ文字を少なくしてください（英語・日本語問わず）。**かわいくて親しみやすい、すっきりとした手描き風のベクターイラスト。配色。丸い頭と棒のような体を持つシンプルな人間のフィギュアと、頭に微妙なグラデーションがある親しみやすい未来的なロボットが登場します。すべてのオブジェクトとキャラクターはシンプルで明確な詳細を持ち、手描きの感触があります。全体的な見た目はミニマルですっきりしており、わかりやすい解説グラフィックに適しています。」とする。
   - **画像サイズ**: 保存・アップロードの際にはファイルサイズが **500KB以下** になるよう配慮（必要に応じてリサイズや圧縮を検討）してください。
...
4. **画像のGitHubプッシュ**:
   ...
   - CDN URLは必ず中継サーバーのキャッシュを回避するため raw リンクを使用する（または安定したリンクを使用）: `https://cdn.jsdelivr.net/gh/dzr01145/shinji_ota@main/public/images/post-<slug>.png`

5. **記事への画像紐付け**:
   - Supabaseへアクセスし、対象記事レコードの `image_url` に上記のURLを設定する。
